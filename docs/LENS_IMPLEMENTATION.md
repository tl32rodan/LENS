...
這是建議給 AI agent 的實作順序。每步完成才做下一步。

### Week 1
1. Repo skeleton、pyproject.toml、CI 基本設定、pre-commit hooks
2. Docker Compose（Kafka + PostgreSQL）本地可跑
3. `lens.events.schema` — `EventEnvelope` + `NodeStarted` + `NodeCompleted` + `FlowStarted` + `FlowCompleted` + `FlowFailed` 的 Pydantic model（完整 TDD）

### Week 2
4. `lens.events.registry` — SchemaRegistry + JSON Schema 檔（完整 TDD）
5. `lens.config` — Settings（TDD）
6. `lens.backbone.bus` — `EventBus`/`EventProducer`/`EventConsumer` Protocols + `InMemoryEventBus` fake（TDD with unit tests using the fake itself; this is what unlocks fast unit testing for Weeks 3-5）

### Week 3
7. `lens.backbone.kafka_bus` — `KafkaEventBus` production adapter (integration tests against testcontainer Kafka)
8. `lens.observer.ap_bridge` — CSVStatusSource + APEventBridge 的 diff 邏輯（完整 TDD，用 InMemoryEventBus）

### Week 4
9. `lens.projection.base` — ProjectionConsumer base class with dedup
10. `lens.projection.dashboard_state` — DashboardStateProjection（TDD with `InMemoryDashboardStateStore` fake; no PostgreSQL needed for unit tests）
11. `lens.projection.dashboard_state_postgres` — Postgres adapter (integration tests against testcontainer PG)
12. PostgreSQL migration（Alembic）

### Week 5（視進度）
12. `lens.api.app` — FastAPI app with basic endpoints
13. 第一個 integration test（Observer → Kafka → Projection → API）
14. UI wireframe 轉成初版 React component

### Week 6（視進度 + MVP polish）
15. 整體 e2e test pass
16. Deployment script
17. Phase 0 demo readiness
...
*— End of LENS Implementation Reference v1.0 —*
